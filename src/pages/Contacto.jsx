function Contacto() {
  ...
  const cardClasses =
    'Card flex flex-wrap items-center justify-center scale-95 hover:scale-100 duration-300 ease-in-out bg-customLigth-blue shadow-md shadow-customDark-blue border border-customSoft-blue h-64 w-60 rounded-lg px-2';
  ...
  return (
    <div className="w-full h-full flex flex-col bg-customWhite items-center justify-start py-2">
      ...
      <div className="flex flex-wrap w-3xl h-full justify-around py-2 mb-10 shadow-md shadow-customGray rounded-lg">
        {socialIcons.map(({ icon, hoverBorder, title, link }, index) => (
          <div key={index} className={`${cardClasses} ${hoverBorder}`}>
            ...
            <a
              href={link}
              target="_blank"
              className="text-center font-semibold text-lg bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors duration-300 no-underline"
            >
              {title}
            </a>
          </div>
      ...
      {/* Mapa */}
      <div className=" w-3xl h-full flex flex-wrap justify-center py-2 mb-5 scale-3xl hover:scale-103 duration-300 ease-in-out shadow-lg shadow-customGray rounded-lg">
        <iframe
